...
- The oldest supported Python version is specified in [doc/dependencies.md](/doc/dependencies.md).
  ...
  The CI linter job also checks this, but [possibly not in all cases](https://github.com/alphacash/alphacash/pull/14884#discussion_r239585126).
...
- Use a module-level docstring to describe what the test is testing, and how it
  is testing it.
- When subclassing the AlphacashTestFramework, place overrides for the
...
- Use `platform.system()` for detecting the running operating system and `os.name` to
  ...
  methods in the `AlphacashTestFramework` class, which can be used to skip a whole test
...
with the alphacashd(s) being tested in a callback-based event loop; the other
implements the test logic.

- `P2PConnection` is the class used to connect to a alphacashd.  `P2PInterface`
contains the higher level logic for processing P2P payloads and connecting to
the Alphacash Core node application logic. For custom behaviour, subclass the
...
The [`TestShell`](test-shell.md) class exposes the AlphacashTestFramework
...
Taken from the [python-alphacashrpc repository](https://github.com/jgarzik/python-alphacashrpc).
...
Test objects for interacting with a alphacashd node over the p2p interface.

#### [script.py](test_framework/script.py)
Utilities for manipulating transaction scripts (originally from python-alphacashlib)
