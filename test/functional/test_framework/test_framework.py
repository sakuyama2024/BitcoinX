...
TMPDIR_PREFIX = "alphacash_func_test_"
...
class AlphacashTestMetaClass(type):
    """Metaclass for AlphacashTestFramework.

    Ensures that any attempt to register a subclass of `AlphacashTestFramework`
    adheres to a standard whereby the subclass overrides `set_test_params` and
    `run_test` but DOES NOT override either `__init__` or `main`. If any of
    those standards are violated, a ``TypeError`` is raised."""

    def __new__(cls, clsname, bases, dct):
        if not clsname == 'AlphacashTestFramework':
            if not ('run_test' in dct and 'set_test_params' in dct):
                raise TypeError("AlphacashTestFramework subclasses must override "
                                "'run_test' and 'set_test_params'")
            if '__init__' in dct or 'main' in dct:
                raise TypeError("AlphacashTestFramework subclasses may not override "
                                "'__init__' or 'main'")

        return super().__new__(cls, clsname, bases, dct)


class AlphacashTestFramework(metaclass=AlphacashTestMetaClass):
    """Base class for a alphacash test script.

    Individual alphacash test scripts should subclass this class and override the set_test_params() and run_test() methods.
    ...
    def parse_args(self):
        ...
        parser.add_argument("--nocleanup", dest="nocleanup", default=False, action="store_true",
                            help="Leave alphacashds and test.* datadir on exit or error")
        parser.add_argument("--noshutdown", dest="noshutdown", default=False, action="store_true",
                            help="Don't stop alphacashds after the test execution")
        ...
        parser.add_argument("--usecli", dest="usecli", default=False, action="store_true",
                            help="use alphacash-cli instead of RPC for all commands")
    ...
    def set_binary_paths(self):
        ...
        binaries = {
            "alphacashd": ("alphacashd", "ALPHACASHD"),
            "alphacash-cli": ("alphacashcli", "ALPHACASHCLI"),
            "alphacash-util": ("alphacashutil", "ALPHACASHUTIL"),
            "alphacash-wallet": ("alphacashwallet", "ALPHACASHWALLET"),
        }
    ...
    def shutdown(self):
        ...
        if not self.options.noshutdown:
            ...
            self.log.info("Note: alphacashds were not stopped and may still be running")
        ...
        rpc_logger = logging.getLogger("AlphacashRPC")
    ...
    def add_nodes(self, num_nodes: int, extra_args=None, *, rpchost=None, binary=None, binary_cli=None, versions=None):
        ...
        if binary is None:
            binary = [get_bin_from_version(v, 'alphacashd', self.options.alphacashd) for v in versions]
        if binary_cli is None:
            binary_cli = [get_bin_from_version(v, 'alphacash-cli', self.options.alphacashcli) for v in versions]
    ...
    def stop_node(self, i, expected_stderr='', wait=0):
        """Stop a alphacashd test node"""
        self.nodes[i].stop_node(expected_stderr, wait=wait)

    def stop_nodes(self, wait=0):
        """Stop multiple alphacashd test nodes"""
        for node in self.nodes:
    ...
    def create_outpoints(self, node, *, outputs):
        """Send funds to a given list of `{address: amount}` targets using the alphacashd
    ...
    def _start_logging(self):
        ...
        ch.setLevel(ll)
        # Format logs the same as alphacashd's debug.log with microprecision (so log files can be concatenated and sorted)
    ...
    def _initialize_chain(self):
        ...
        if not os.path.isdir(cache_node_dir):
            ...
            self.nodes.append(
                TestNode(
                    ...
                    timewait=self.rpc_timeout,
                    timeout_factor=self.options.timeout_factor,
                    alphacashd=self.options.alphacashd,
                    alphacash_cli=self.options.alphacashcli,
                    coverage_dir=None,
                    cwd=self.options.tmpdir,
                    descriptors=self.options.descriptors,
                ))
        ...
        for i in range(self.num_nodes):
            ...
            initialize_datadir(self.options.tmpdir, i, self.chain, self.disable_autoconnect)  # Overwrite port/rpcport in alphacash.conf
    ...
    def skip_if_no_alphacashd_tracepoints(self):
        """Skip the running test if alphacashd has not been compiled with USDT tracepoint support."""
        if not self.is_usdt_compiled():
            raise SkipTest("alphacashd has not been built with USDT tracepoints enabled.")
    ...
    def skip_if_no_alphacashd_zmq(self):
        """Skip the running test if alphacashd has not been compiled with zmq support."""
        if not self.is_zmq_compiled():
            raise SkipTest("alphacashd has not been built with zmq enabled.")
    ...
    def skip_if_no_wallet_tool(self):
        """Skip the running test if alphacash-wallet has not been compiled."""
        if not self.is_wallet_tool_compiled():
            raise SkipTest("alphacash-wallet has not been compiled")

    def skip_if_no_alphacash_util(self):
        """Skip the running test if alphacash-util has not been compiled."""
        if not self.is_alphacash_util_compiled():
            raise SkipTest("alphacash-util has not been compiled")

    def skip_if_no_cli(self):
        """Skip the running test if alphacash-cli has not been compiled."""
        if not self.is_cli_compiled():
            raise SkipTest("alphacash-cli has not been compiled.")
    ...
    def is_cli_compiled(self):
        """Checks whether alphacash-cli was compiled."""
    ...
    def is_wallet_tool_compiled(self):
        """Checks whether alphacash-wallet was compiled."""
        return self.config["components"].getboolean("ENABLE_WALLET_TOOL")

    def is_alphacash_util_compiled(self):
        """Checks whether alphacash-util was compiled."""
        return self.config["components"].getboolean("ENABLE_ALPHACASH_UTIL")
