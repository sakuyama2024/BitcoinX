...
`test/functional/test_framework/test_framework.py:AlphacashTestFramework.main`.
...
BASE_SCRIPTS = [
    ...
    'interface_alphacash_cli.py --legacy-wallet',
    'interface_alphacash_cli.py --descriptors',
...
def main():
    ...
    enable_alphacashd = config["components"].getboolean("ENABLE_ALPHACASHD")

    if not enable_alphacashd:
...
def run_tests(*, test_list, src_dir, build_dir, tmpdir, jobs=1, enable_coverage=False, args=None, combined_logs_len=0, failfast=False, use_term_control):
    ...
    # Warn if alphacashd is already running
    try:
        # pgrep exits with code zero when one or more matching processes found
        if subprocess.run(["pgrep", "-x", "alphacashd"], stdout=subprocess.DEVNULL).returncode == 0:
            print("%sWARNING!%s There is already a alphacashd process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
    except OSError:
        # pgrep not supported
        pass
    ...
    if shutil.disk_usage(tmpdir).free < min_space:
        print(f"{BOLD[1]}WARNING!{BOLD[0]} There may be insufficient free space in {tmpdir} to run the Alphacash functional test suite. "
    ...
    # a hard link or a copy on any platform. See https://github.com/alphacash/alphacash/pull/27561.
...
class RPCCoverage():
    ...
    commands per `alphacash-cli help` (`rpc_interface.txt`).
