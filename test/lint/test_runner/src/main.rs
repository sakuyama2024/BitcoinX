...
fn get_pathspecs_exclude_whitespace() -> Vec<String> {
    ...
    list.extend(
        [
            ...
            "contrib/init/alphacashd.openrc",
...
fn lint_includes_build_config() -> LintResult {
    let config_path = "./src/config/alphacash-config.h.in";
    ...
    let print_affected_files = |mode: bool| {
        ...
        let defines_files = check_output(
            git()
                ...
                .args([
                    // These are exceptions which don't use alphacash-config.h, rather the Makefile.am adds
        ...
        git()
            .args([
                ...
                if mode {
                    ...
                    "#include <config/alphacash-config.h>" // Catch redundant includes with and without the IWYU pragma
                },
                "--",
            ])
    ...
    if missing {
        return Err(format!(
            r#"
^^^
One or more files use a symbol declared in the alphacash-config.h header. However, they are not
...
even though alphacash-config.h indicates that a faster feature is available and should be used.
...
#include <config/alphacash-config.h> // IWYU pragma: keep
    ...
    if redundant {
        return Err(r#"
^^^
None of the files use a symbol declared in the alphacash-config.h header. However, they are including
