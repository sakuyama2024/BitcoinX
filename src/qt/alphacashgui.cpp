...
#include <config/alphacash-config.h> // IWYU pragma: keep

#include <qt/alphacashgui.h>

#include <qt/alphacashunits.h>
...
const std::string AlphacashGUI::DEFAULT_UIPLATFORM =
...
AlphacashGUI::AlphacashGUI(interfaces::Node& node, const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    m_node(node),
    ...
    m_network_style(networkStyle)
{
    ...
    if(enableWallet)
    {
        ...
        connect(walletFrame, &WalletFrame::createWalletButtonClicked, this, &AlphacashGUI::createWallet);
    ...
    connect(labelBlocksIcon, &GUIUtil::ClickableLabel::clicked, this, &AlphacashGUI::showModalOverlay);
    connect(progressBar, &GUIUtil::ClickableProgressBar::clicked, this, &AlphacashGUI::showModalOverlay);
...
void AlphacashGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Alphacash address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and alphacash: URIs)"));
    ...
    connect(overviewAction, &QAction::triggered, this, &AlphacashGUI::gotoOverviewPage);
    connect(sendCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(sendCoinsAction, &QAction::triggered, [this]{ gotoSendCoinsPage(); });
    connect(receiveCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(receiveCoinsAction, &QAction::triggered, this, &AlphacashGUI::gotoReceiveCoinsPage);
    connect(historyAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(historyAction, &QAction::triggered, this, &AlphacashGUI::gotoHistoryPage);
#endif // ENABLE_WALLET
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Alphacash addresses to prove you own them"));
    verifyMessageAction = new QAction(tr("&Verify message…"), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Alphacash addresses"));
    m_load_psbt_action = new QAction(tr("&Load PSBT from file…"), this);
    m_load_psbt_action->setStatusTip(tr("Load Partially Signed Alphacash Transaction"));
    m_load_psbt_clipboard_action = new QAction(tr("Load PSBT from &clipboard…"), this);
    m_load_psbt_clipboard_action->setStatusTip(tr("Load Partially Signed Alphacash Transaction from clipboard"));
    ...
    openAction->setStatusTip(tr("Open a alphacash: URI"));
    ...
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Alphacash command-line options").arg(PACKAGE_NAME));
    ...
    connect(quitAction, &QAction::triggered, this, &AlphacashGUI::quitRequested);
    connect(aboutAction, &QAction::triggered, this, &AlphacashGUI::aboutClicked);
    connect(aboutQtAction, &QAction::triggered, qApp, QApplication::aboutQt);
    connect(optionsAction, &QAction::triggered, this, &AlphacashGUI::optionsClicked);
    connect(showHelpMessageAction, &QAction::triggered, this, &AlphacashGUI::showHelpMessageClicked);
    connect(openRPCConsoleAction, &QAction::triggered, this, &AlphacashGUI::showDebugWindow);
    ...
    if(walletFrame)
    {
        ...
        connect(openAction, &QAction::triggered, this, &AlphacashGUI::openClicked);
        connect(m_open_wallet_menu, &QMenu::aboutToShow, [this] {
            ...
            for (const std::pair<const std::string, bool>& i : m_wallet_controller->listWalletDir()) {
                ...
                connect(action, &QAction::triggered, [this, path] {
                    ...
                    connect(activity, &OpenWalletActivity::opened, this, &AlphacashGUI::setCurrentWallet, Qt::QueuedConnection);
        ...
        connect(m_restore_wallet_action, &QAction::triggered, [this] {
            ...
            connect(activity, &RestoreWalletActivity::restored, this, &AlphacashGUI::setCurrentWallet, Qt::QueuedConnection);
        ...
        connect(m_create_wallet_action, &QAction::triggered, this, &AlphacashGUI::createWallet);
        connect(m_close_all_wallets_action, &QAction::triggered, [this] {
            m_wallet_controller->closeAllWallets(this);
        });
        connect(m_migrate_wallet_action, &QAction::triggered, [this] {
            auto activity = new MigrateWalletActivity(m_wallet_controller, this);
            connect(activity, &MigrateWalletActivity::migrated, this, &AlphacashGUI::setCurrentWallet);
            activity->migrate(walletFrame->currentWalletModel());
        });
        connect(m_mask_values_action, &QAction::toggled, this, &AlphacashGUI::setPrivacy);
        connect(m_mask_values_action, &QAction::toggled, this, &AlphacashGUI::enableHistoryAction);
    }
#endif // ENABLE_WALLET

    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_C), this), &QShortcut::activated, this, &AlphacashGUI::showDebugWindowActivateConsole);
    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_D), this), &QShortcut::activated, this, &AlphacashGUI::showDebugWindow);
}

void AlphacashGUI::createMenuBar()
{
    appMenuBar = menuBar();
...
void AlphacashGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        connect(m_wallet_selector, qOverload<int>(&QComboBox::currentIndexChanged), this, &AlphacashGUI::setCurrentWalletBySelectorIndex);
...
void AlphacashGUI::setClientModel(ClientModel *_clientModel, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    if(_clientModel)
    {
        ...
        connect(_clientModel, &ClientModel::numConnectionsChanged, this, &AlphacashGUI::setNumConnections);
        connect(_clientModel, &ClientModel::networkActiveChanged, this, &AlphacashGUI::setNetworkActive);
        ...
        connect(_clientModel, &ClientModel::numBlocksChanged, this, &AlphacashGUI::setNumBlocks);
        ...
        connect(_clientModel, &ClientModel::showProgress, this, &AlphacashGUI::showProgress);
...
#ifdef ENABLE_WALLET
void AlphacashGUI::enableHistoryAction(bool privacy)
{
...
void AlphacashGUI::setWalletController(WalletController* wallet_controller, bool show_loading_minimized)
{
    ...
    GUIUtil::ExceptionSafeConnect(wallet_controller, &WalletController::walletAdded, this, &AlphacashGUI::addWallet);
    connect(wallet_controller, &WalletController::walletRemoved, this, &AlphacashGUI::removeWallet);
...
WalletController* AlphacashGUI::getWalletController()
{
    return m_wallet_controller;
}

void AlphacashGUI::addWallet(WalletModel* walletModel)
{
    ...
    connect(wallet_view, &WalletView::outOfSyncWarningClicked, this, &AlphacashGUI::showModalOverlay);
    connect(wallet_view, &WalletView::transactionClicked, this, &AlphacashGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::coinsSent, this, &AlphacashGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::message, [this](const QString& title, const QString& message, unsigned int style) {
        this->message(title, message, style);
    });
    connect(wallet_view, &WalletView::encryptionStatusChanged, this, &AlphacashGUI::updateWalletStatus);
    connect(wallet_view, &WalletView::incomingTransaction, this, &AlphacashGUI::incomingTransaction);
    connect(this, &AlphacashGUI::setPrivacy, wallet_view, &WalletView::setPrivacy);
...
void AlphacashGUI::removeWallet(WalletModel* walletModel)
{
...
void AlphacashGUI::setCurrentWallet(WalletModel* wallet_model)
{
    if (!walletFrame || !m_wallet_controller) return;
...
void AlphacashGUI::setCurrentWalletBySelectorIndex(int index)
{
    WalletModel* wallet_model = m_wallet_selector->itemData(index).value<WalletModel*>();
    if (wallet_model) setCurrentWallet(wallet_model);
}

void AlphacashGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void AlphacashGUI::setWalletActionsEnabled(bool enabled)
{
...
void AlphacashGUI::createTrayIcon()
{
...
void AlphacashGUI::createTrayIconMenu()
{
#ifndef Q_OS_MACOS
    if (!trayIcon) return;
#endif // Q_OS_MACOS
    ...
    show_hide_action = trayIconMenu->addAction(QString(), this, &AlphacashGUI::toggleHidden);
...
void AlphacashGUI::optionsClicked()
{
    openOptionsDialogWithTab(OptionsDialog::TAB_MAIN);
}

void AlphacashGUI::aboutClicked()
{
    if(!clientModel)
        return;
...
void AlphacashGUI::showDebugWindow()
{
    GUIUtil::bringToFront(rpcConsole);
    Q_EMIT consoleShown(rpcConsole);
}

void AlphacashGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TabTypes::CONSOLE);
    showDebugWindow();
}

void AlphacashGUI::showHelpMessageClicked()
{
    GUIUtil::bringToFront(helpMessageDialog);
}

#ifdef ENABLE_WALLET
void AlphacashGUI::openClicked()
{
...
void AlphacashGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void AlphacashGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void AlphacashGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void AlphacashGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void AlphacashGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void AlphacashGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}
void AlphacashGUI::gotoLoadPSBT(bool from_clipboard)
{
    if (walletFrame) walletFrame->gotoLoadPSBT(from_clipboard);
}
#endif // ENABLE_WALLET

void AlphacashGUI::updateNetworkState()
{
    ...
    if (m_node.getNetworkActive()) {
        ...
        tooltip = tr("%n active connection(s) to Alphacash network.", "", count);
    } else {
        //: A substring of the tooltip.
...
void AlphacashGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void AlphacashGUI::setNetworkActive(bool network_active)
{
    updateNetworkState();
...
void AlphacashGUI::updateHeadersSyncProgressLabel()
{
...
void AlphacashGUI::updateHeadersPresyncProgressLabel(int64_t height, const QDateTime& blockDate)
{
...
void AlphacashGUI::openOptionsDialogWithTab(OptionsDialog::Tab tab)
{
    if (!clientModel || !clientModel->getOptionsModel())
        return;

    auto dlg = new OptionsDialog(this, enableWallet);
    connect(dlg, &OptionsDialog::quitOnReset, this, &AlphacashGUI::quitRequested);
...
void AlphacashGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, SyncType synctype, SynchronizationState sync_state)
{
...
void AlphacashGUI::createWallet()
{
#ifdef ENABLE_WALLET
#ifndef USE_SQLITE
    ...
    connect(activity, &CreateWalletActivity::created, this, &AlphacashGUI::setCurrentWallet);
    connect(activity, &CreateWalletActivity::created, rpcConsole, &RPCConsole::setCurrentWallet);
    activity->create();
#endif // ENABLE_WALLET
}

void AlphacashGUI::message(const QString& title, QString message, unsigned int style, bool* ret, const QString& detailed_message)
{
...
void AlphacashGUI::changeEvent(QEvent *e)
{
    ...
    if(e->type() == QEvent::WindowStateChange)
    {
        if(clientModel && clientModel->getOptionsModel() && clientModel->getOptionsModel()->getMinimizeToTray())
        {
            ...
            {
                QTimer::singleShot(0, this, &AlphacashGUI::hide);
                e->ignore();
            }
            else if((wsevt->oldState() & Qt::WindowMinimized) && !isMinimized())
            {
                QTimer::singleShot(0, this, &AlphacashGUI::show);
                e->ignore();
            }
        }
    }
...
void AlphacashGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MACOS // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void AlphacashGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void AlphacashGUI::incomingTransaction(const QString& date, AlphacashUnit unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& walletName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date) +
                  tr("Amount: %1\n").arg(AlphacashUnits::formatWithUnit(unit, amount, true));
...
void AlphacashGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void AlphacashGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool AlphacashGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool AlphacashGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void AlphacashGUI::setHDStatus(bool privkeyDisabled, int hdEnabled)
{
...
void AlphacashGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void AlphacashGUI::updateWalletStatus()
{
    assert(walletFrame);
...
void AlphacashGUI::updateProxyIcon()
{
    std::string ip_port;
...
void AlphacashGUI::updateWindowTitle()
{
...
void AlphacashGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void AlphacashGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void AlphacashGUI::detectShutdown()
{
...
void AlphacashGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0) {
...
void AlphacashGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(AlphacashGUI* gui, const bilingual_str& message, const std::string& caption, unsigned int style)
{
    ...
    if (message.original != message.translated) {
        detailed_message = AlphacashGUI::tr("Original message:") + "\n" + QString::fromStdString(message.original);
    }
...
void AlphacashGUI::subscribeToCoreSignals()
{
...
void AlphacashGUI::unsubscribeFromCoreSignals()
{
...
bool AlphacashGUI::isPrivacyModeActivated() const
{
    ...
    : m_platform_style{platformStyle}
{
    ...
    QList<AlphacashUnit> units = AlphacashUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const AlphacashUnit unit : units) {
        max_width = qMax(max_width, GUIUtil::TextWidth(fm, AlphacashUnits::longName(unit)));
    }
...
{
    menu = new QMenu(this);
    for (const AlphacashUnit u : AlphacashUnits::availableUnits()) {
        menu->addAction(AlphacashUnits::longName(u))->setData(QVariant::fromValue(u));
    }
...
void UnitDisplayStatusBarControl::updateDisplayUnit(AlphacashUnit newUnits)
{
    setText(AlphacashUnits::longName(newUnits));
}
