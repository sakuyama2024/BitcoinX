...
#include <qt/alphacashaddressvalidator.h>
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Alphacash address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
    widget->setValidator(new AlphacashAddressEntryValidator(parent));
    widget->setCheckValidator(new AlphacashAddressCheckValidator(parent));
}
...
bool parseAlphacashURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no alphacash: URI
    if(!uri.isValid() || uri.scheme() != QString("alphacash"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if (!AlphacashUnits::parse(AlphacashUnit::BTC, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
bool parseAlphacashURI(QString uri, SendCoinsRecipient *out)
{
    QUrl uriInstance(uri);
    return parseAlphacashURI(uriInstance, out);
}

QString formatAlphacashURI(const SendCoinsRecipient &info)
{
    bool bech_32 = info.address.startsWith(QString::fromStdString(Params().Bech32HRP() + "1"));

    QString ret = QString("alphacash:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(AlphacashUnits::format(AlphacashUnit::BTC, info.amount, false, AlphacashUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
bool openAlphacashConf()
{
    ...
    /* Open alphacash.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == ChainType::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Alphacash.lnk";
    if (chain == ChainType::TESTNET) // Remove this special case when testnet CBaseChainParams::DataDir() is incremented to "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Alphacash (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / fs::u8path(strprintf("Alphacash (%s).lnk", ChainTypeToString(chain)));
}

bool GetStartOnSystemStartup()
{
    // check for Alphacash*.lnk
...
fs::path static GetAutostartFilePath()
{
    ChainType chain = gArgs.GetChainType();
    if (chain == ChainType::MAIN)
        return GetAutostartDir() / "alphacash.desktop";
    return GetAutostartDir() / fs::u8path(strprintf("alphacash-%s.desktop", ChainTypeToString(chain)));
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        ChainType chain = gArgs.GetChainType();
        // Write a alphacash.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == ChainType::MAIN)
            optionFile << "Name=Alphacash\n";
        else
            optionFile << strprintf("Name=Alphacash (%s)\n", ChainTypeToString(chain));
