...
/** Call node.updateRwSetting() with Alphacash 22.x workaround. */
static void UpdateRwSetting(interfaces::Node& node, OptionsModel::OptionID option, const std::string& suffix, const common::SettingsValue& value)
{
    if (value.isNum() &&
        ...
        // because Alphacash 22.x releases try to read these specific settings as
        ...
        // in later releases by https://github.com/alphacash/alphacash/pull/24498.
        // If new numeric settings are added, they can be written as numbers
        // instead of strings, because alphacash 22.x will not try to read these.
...
//! Convert enabled/size values to alphacash -prune setting.
...
//! Get pruning enabled value to show in GUI from alphacash -prune setting.
...
//! Get pruning size value to show in GUI from alphacash -prune setting. If
...
bool OptionsModel::Init(bilingual_str& error)
{
    ...
    // Display
    if (!settings.contains("DisplayAlphacashUnit")) {
        settings.setValue("DisplayAlphacashUnit", QVariant::fromValue(AlphacashUnit::BTC));
    }
    QVariant unit = settings.value("DisplayAlphacashUnit");
    if (unit.canConvert<AlphacashUnit>()) {
        m_display_alphacash_unit = unit.value<AlphacashUnit>();
    } else {
        m_display_alphacash_unit = AlphacashUnit::BTC;
        settings.setValue("DisplayAlphacashUnit", QVariant::fromValue(m_display_alphacash_unit));
    }
...
void OptionsModel::SetPruneTargetGB(int prune_target_gb)
{
    ...
    // from saved value. Avoid writing settings.json if alphacash.conf value
...
#endif
    case DisplayUnit:
        return QVariant::fromValue(m_display_alphacash_unit);
...
{
    if (new_unit.isNull() || new_unit.value<AlphacashUnit>() == m_display_alphacash_unit) return;
    m_display_alphacash_unit = new_unit.value<AlphacashUnit>();
    QSettings settings;
    settings.setValue("DisplayAlphacashUnit", QVariant::fromValue(m_display_alphacash_unit));
    Q_EMIT displayUnitChanged(m_display_alphacash_unit);
}
...
void OptionsModel::checkAndMigrate()
{
    ...
    if (settingsVersion < CLIENT_VERSION)
    {
        ...
        // see https://github.com/alphacash/alphacash/pull/8273
    ...
    // (https://github.com/alphacash-core/gui/issues/567).
