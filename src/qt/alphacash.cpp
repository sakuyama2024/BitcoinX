...
static void RegisterMetaTypes()
{
    ...
    qRegisterMetaTypeStreamOperators<AlphacashUnit>("AlphacashUnit");
#else
    qRegisterMetaType<AlphacashUnit>("AlphacashUnit");
#endif
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. alphacash_de.qm (shortcut "de" needs to be defined in alphacash.qrc)
    if (translatorBase.load(lang, ":/translations/")) {
        QApplication::installTranslator(&translatorBase);
    }

    // Load e.g. alphacash_de_DE.qm (shortcut "de_DE" needs to be defined in alphacash.qrc)
...
static const char* qt_argv = "alphacash-qt";

AlphacashApplication::AlphacashApplication()
...
void AlphacashApplication::setupPlatformStyle()
{
    // UI per-platform customization
    // This must be done inside the AlphacashApplication constructor, or after it, because
    // PlatformStyle::instantiate requires a QApplication
    std::string platformName;
    platformName = gArgs.GetArg("-uiplatform", AlphacashGUI::DEFAULT_UIPLATFORM);
...
#ifdef ENABLE_WALLET
void AlphacashApplication::createPaymentServer()
{
    paymentServer = new PaymentServer(this);
}
#endif

bool AlphacashApplication::createOptionsModel(bool resetSettings)
{
...
void AlphacashApplication::createWindow(const NetworkStyle *networkStyle)
{
    window = new AlphacashGUI(node(), platformStyle, networkStyle, nullptr);
    connect(window, &AlphacashGUI::quitRequested, this, &AlphacashApplication::requestShutdown);
...
void AlphacashApplication::createSplashScreen(const NetworkStyle *networkStyle)
{
    assert(!m_splash);
...
void AlphacashApplication::createNode(interfaces::Init& init)
{
    assert(!m_node);
...
bool AlphacashApplication::baseInitialize()
{
    return node().baseInitialize();
}

void AlphacashApplication::startThread()
{
    assert(!m_executor);
    m_executor.emplace(node());

    /*  communication to and from thread */
    connect(&m_executor.value(), &InitExecutor::initializeResult, this, &AlphacashApplication::initializeResult);
    connect(&m_executor.value(), &InitExecutor::shutdownResult, this, [] {
        QCoreApplication::exit(0);
    });
    connect(&m_executor.value(), &InitExecutor::runawayException, this, &AlphacashApplication::handleRunawayException);
    connect(this, &AlphacashApplication::requestedInitialize, &m_executor.value(), &InitExecutor::initialize);
    connect(this, &AlphacashApplication::requestedShutdown, &m_executor.value(), &InitExecutor::shutdown);
}

void AlphacashApplication::parameterSetup()
{
...
void AlphacashApplication::InitPruneSetting(int64_t prune_MiB)
{
    optionsModel->SetPruneTargetGB(PruneMiBtoGB(prune_MiB));
}

void AlphacashApplication::requestInitialize()
{
...
void AlphacashApplication::requestShutdown()
{
...
void AlphacashApplication::initializeResult(bool success, interfaces::BlockAndHeaderTipInfo tip_info)
{
    ...
    if (success) {
        ...
        // alphacash: URIs or payment requests:
        if (paymentServer) {
            connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &AlphacashGUI::handlePaymentRequest);
            connect(window, &AlphacashGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
void AlphacashApplication::handleRunawayException(const QString &message)
{
...
void AlphacashApplication::handleNonFatalException(const QString& message)
{
...
WId AlphacashApplication::getMainWinId() const
{
    if (!window)
        return 0;
...
bool AlphacashApplication::event(QEvent* e)
{
...
static void SetupUIArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", AlphacashGUI::DEFAULT_UIPLATFORM), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::GUI);
}
...
int GuiMain(int argc, char* argv[])
{
    ...
    Q_INIT_RESOURCE(alphacash);
    Q_INIT_RESOURCE(alphacash_locale);
    ...
    for (int i = 1; i < argc; i++) {
...
#ifdef ENABLE_WALLET
        if (arg.startsWith(ALPHACASH_IPC_PREFIX, Qt::CaseInsensitive)) {
            invalid_token &= false;
        ...
        if (invalid_token) {
            InitError(Untranslated(strprintf("Command line contains unexpected token '%s', see alphacash-qt -h for a list of options.", argv[i])));
            QMessageBox::critical(nullptr, PACKAGE_NAME,
                                  // message cannot be translated because translations have not been initialized
                                  QString::fromStdString("Command line contains unexpected token '%1', see alphacash-qt -h for a list of options.").arg(QString::fromStdString(argv[i])));
            return EXIT_FAILURE;
        }
    }
    ...
    /// 6-7. Parse alphacash.conf, determine network, switch to network specific
    ...
    // alphacash: links repeatedly have their payment requests routed to this process:
    if (WalletModel::isWalletEnabled()) {
