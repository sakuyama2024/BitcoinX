...
const char * const ALPHACASH_CONF_FILENAME = "alphacash.conf";
const char * const ALPHACASH_SETTINGS_FILENAME = "settings.json";
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //alphacash-tx using stdin
...
{
    fs::path settings = GetPathArg("-settings", ALPHACASH_SETTINGS_FILENAME);
    if (settings.empty()) {
        return false;
    }
    if (backup) {
...
{
    // Windows: C:\Users\Username\AppData\Roaming\Alphacash
    // macOS: ~/Library/Application Support/Alphacash
    // Unix-like: ~/.alphacash
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Alphacash";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/Alphacash";
#else
    // Unix-like
    return pathRet / ".alphacash";
#endif
#endif
}
