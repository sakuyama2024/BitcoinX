...
parser = argparse.ArgumentParser(description='Script to get coins from a faucet.', epilog='You may need to start with double-dash (--) when providing alphacash-cli arguments.')
parser.add_argument('-c', '--cmd', dest='cmd', default='alphacash-cli', help='alphacash-cli command to use')
...
parser.add_argument('-a', '--addr', dest='addr', default='', help='Alphacash address to which the faucet should send')
...
parser.add_argument('alphacash_cli_args', nargs='*', help='Arguments to pass on to alphacash-cli (default: -signet)')

args = parser.parse_args()

if args.alphacash_cli_args == []:
    args.alphacash_cli_args = ['-signet']


def alphacash_cli(rpc_command_and_params):
    argv = [args.cmd] + args.alphacash_cli_args + rpc_command_and_params
    try:
...
if args.faucet.lower() == DEFAULT_GLOBAL_FAUCET:
    ...
    curr_signet_hash = alphacash_cli(['getblockhash', '1'])
...
if args.addr == '':
    ...
    args.addr = alphacash_cli(['getnewaddress', 'faucet', 'bech32'])
