...
"""Script for verifying Alphacash Core release binaries.

This script attempts to download the sum file SHA256SUMS and corresponding
signature file SHA256SUMS.asc from alphacashcore.org and alphacash.org and
compares them.
...
The builder keys are available in the guix.sigs repo:
    ...
    https://github.com/alphacash-core/guix.sigs/tree/main/builder-keys
...
HOST1 = "https://alphacashcore.org"
HOST2 = "https://alphacash.org"
VERSIONPREFIX = "alphacash-core-"
...
def verify_shasums_signature(
   ...
   ReturnCode, list[SigData], list[SigData], list[SigData], list[SigData]
    ...
    # which pubkeys within the Alphacash community do we trust for the purposes of
    # binary verification?
    trusted_keys = set()
...
def verify_published_handler(args: argparse.Namespace) -> ReturnCode:
    WORKINGDIR = Path(tempfile.gettempdir()) / f"alphacash_verify_binaries.{args.version}"
    ...
    # remove binaries that are known not to be hosted by alphacashcore.org
...
def main():
    ...
    pub_parser.add_argument(
        'version', type=str, help=(
            f'version of the alphacash release to download; of the format '
    ...
    pub_parser.add_argument(
        ...
        help=(
            ...
            '(Sometimes alphacash.org lags behind alphacashcore.org.)')
    )
