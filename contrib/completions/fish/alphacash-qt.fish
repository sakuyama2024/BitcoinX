...
complete --command alphacash-qt --no-files

# Extract options
function __fish_alphacashqt_get_options
...
complete \
    --command alphacash-qt \
    --arguments "(__fish_alphacashqt_get_options)"
# Enable file completions only if the commandline now contains a `*.=` style option
complete -c alphacash-qt \
...
complete \
    --command alphacash-qt \
    --arguments "(__fish_alphacashqt_get_options --nofiles)"
