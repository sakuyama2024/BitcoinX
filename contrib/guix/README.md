# Bootstrappable Alphacash Core Builds

This directory contains the files necessary to perform bootstrappable Alphacash
Core builds.
...
To build Alphacash Core reproducibly with all default options, invoke the
...
2. The `guix-codesign` command ***requires*** a `DETACHED_SIGS_REPO` flag.
    * _**DETACHED_SIGS_REPO**_
      ...
      Alphacash Core version being built.
...
```
env DETACHED_SIGS_REPO=<path/to/alphacash-detached-sigs> ./contrib/guix/guix-codesign
```
...
repository](https://github.com/alphacash-core/guix.sigs).
