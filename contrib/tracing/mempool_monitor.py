...
""" Example logging Alphacash Core mempool events using the mempool:added,
...
def main(alphacashd_path):
    alphacashd_with_usdts = USDT(path=str(alphacashd_path))

    # attaching the trace functions defined in the BPF program
    # to the tracepoints
    alphacashd_with_usdts.enable_probe(probe="mempool:added", fn_name="trace_added")
    alphacashd_with_usdts.enable_probe(probe="mempool:removed", fn_name="trace_removed")
    alphacashd_with_usdts.enable_probe(probe="mempool:replaced", fn_name="trace_replaced")
    alphacashd_with_usdts.enable_probe(probe="mempool:rejected", fn_name="trace_rejected")
    bpf = BPF(text=PROGRAM, usdt_contexts=[alphacashd_with_usdts])
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("USAGE: ", sys.argv[0], "path/to/alphacashd")
        exit(1)
