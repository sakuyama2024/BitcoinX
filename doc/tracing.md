# User-space, Statically Defined Tracing (USDT) for Alphacash Core

Alphacash Core includes statically defined tracepoints to allow for more
...
**Note**: Alphacash Core uses temporary clones of the _main_ UTXO cache
...
## Adding tracepoints to Alphacash Core
...
Multiple tools can list the available tracepoints in a `alphacashd` binary with
USDT support.

### GDB - GNU Project Debugger

To list probes in Alphacash Core, use `info probes` in `gdb`:
...
stap net        inbound_message  0x000000000014419e /src/alphacashd
stap net        outbound_message 0x0000000000107c05 /src/alphacashd
stap validation block_connected  0x00000000002fb10c /src/alphacashd
…
```

### With `readelf`

The `readelf` tool can be used to display the USDT tracepoints in Alphacash Core.
Look for the notes with the description `NT_STAPSDT`.

```
$ readelf -n ./src/alphacashd | grep NT_STAPSDT -A 4 -B 2
