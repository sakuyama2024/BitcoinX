# CJDNS support in Alphacash Core

It is possible to run Alphacash Core over CJDNS, an encrypted IPv6 network that
...
done outside of Alphacash Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the Alphacash network and individual nodes.
...
before it will work with your Alphacash Core node. This is described in steps
...
## Run Alphacash Core with CJDNS

Once you are connected to the CJDNS network, the following Alphacash Core
...
When enabled, this option tells Alphacash Core that it is running in an
...
IPv6 local network. This helps Alphacash Core perform better address management:
...
CJDNS support was added to Alphacash Core in version 23.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `alphacash-cli -addrinfo` to see the
...
[doc/tor.md](https://github.com/alphacash/alphacash/blob/master/doc/tor.md) for
details.

## CJDNS-related information in Alphacash Core

There are several ways to see your CJDNS address in Alphacash Core:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `alphacash-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `alphacash-cli getpeerinfo`).

You can use the `getnodeaddresses` RPC to fetch a number of CJDNS peers known to your node; run `alphacash-cli help getnodeaddresses` for details.
