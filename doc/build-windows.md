...
Below are some notes on how to build Alphacash Core for Windows.

The options known to work for building Alphacash Core on Windows are:
...
Note that for WSL the Alphacash Core source path MUST be somewhere in the default mount file system, for
example /usr/src/alphacash, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
...
way. This will install to `c:\workspace\alphacash`, for example:

    make install DESTDIR=/mnt/c/workspace/alphacash
