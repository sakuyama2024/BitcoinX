...
Alphacash Core version 0.21.0 is now available from:

  <https://alphacashcore.org/bin/alphacash-core-0.21.0/>
...
- This release adds support for Tor version 3 hidden services, and rumoring them
  ...
  [BIP155](https://github.com/alphacash/bips/blob/master/bip-0155.mediawiki).
  Version 2 hidden services are still fully supported by Alphacash Core, but the
...
- The Tor onion service that is automatically created by setting the
  ...
  `-datadir`) and can be removed if not needed. Alphacash Core will no longer
...
- This release adds support for serving
  [BIP157](https://github.com/alphacash/bips/blob/master/bip-0157.mediawiki) compact
...
- This release adds support for signets
  ([BIP325](https://github.com/alphacash/bips/blob/master/bip-0325.mediawiki)) in
...
- This release implements
  [BIP339](https://github.com/alphacash/bips/blob/master/bip-0339.mediawiki)
...
- This release implements the proposed Taproot consensus rules
  ([BIP341](https://github.com/alphacash/bips/blob/master/bip-0341.mediawiki) and
  [BIP342](https://github.com/alphacash/bips/blob/master/bip-0342.mediawiki)),
...
- A new `alphacash-cli -netinfo` command provides a network peer connections
...
- A new `alphacash-cli -generate` command, equivalent to RPC `generatenewaddress`
  followed by `generatetoaddress`, can generate blocks for command line testing
  purposes. This is a client-side version of the former `generate` RPC. See the
  help for details. (#19133)

- The `alphacash-cli -getinfo` command now displays the wallet name and balance for
  each of the loaded wallets when more than one is loaded (e.g. in multiwallet
  mode) and a wallet is not specified with `-rpcwallet`. (#18594)

- The `connections` field of `alphacash-cli -getinfo` is now expanded to return a JSON
...
- The `startupnotify` option is used to specify a command to
  execute when Alphacash Core has finished with its startup
  sequence. (#15367)
...
- The `-salvagewallet` startup option has been removed. A new `salvage` command
  has been added to the `alphacash-wallet` tool which performs the salvage
...
Alphacash Core will no longer automatically create new wallets on startup. It will
load existing wallets specified by `-wallet` options on the command line or in
`alphacash.conf` or `settings.json` files. And by default it will also load a
top-level unnamed ("") wallet. However, if specified wallets don't exist,
Alphacash Core will now just log warnings instead of creating new wallets with
new keys and addresses like previous releases did.

New wallets can be created through the GUI (which has a more prominent create
wallet option), through the `alphacash-cli createwallet` or `alphacash-wallet
...
Bugs and missing functionality can be reported to the [issue tracker](https://github.com/alphacash/alphacash/issues).
...
Users who already have a Alphacash Core wallet can continue to use it as they did before without
any change in behavior. Newly created Legacy Wallets (which remains the default type of wallet) will
behave as they did in previous versions of Alphacash Core.
...
The change to using descriptors changes the default derivation paths used by Alphacash Core
...
- Wallets created or loaded in the GUI will now be automatically loaded on
  startup, so they don't need to be manually reloaded next time Alphacash Core is
  started. The list of wallets to load on startup is stored in
  `\<datadir\>/settings.json` and augments any command line or `alphacash.conf`
...
- #19284 Add seed.alphacash.wiz.biz to DNS seeds (wiz)
...
- #20198 Show name, format and if uses descriptors in alphacash-wallet tool (jonasschnelli)
...
- #19133 rpc, cli, test: add alphacash-cli -generate command (jonatack)
...
- #20055 Set HTTP Content-Type in alphacash-cli (laanwj)
...
### Build system
- #18504 Drop alphacash-tx and alphacash-wallet dependencies on libevent (ryanofsky)
...
- #18297 Use pkg-config in ALPHACASH_QT_CONFIGURE for all hosts including Windows (hebasto)
...
- #18653 add coverage for alphacash-cli -rpcwait (jonatack)
...
- #18739 Document how to fuzz Alphacash Core using Honggfuzz (practicalswift)
...
- #19061 Add link to Visual Studio build readme (maitrealphacash)
...
- #18817 Document differences in alphacashd and alphacash-qt locale handling (practicalswift)
